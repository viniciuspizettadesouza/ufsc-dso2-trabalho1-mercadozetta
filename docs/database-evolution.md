...
Migration `0008` keeps `mercadozetta` authoritative at `USD`/`2` and changes
only `campus-market` to `EUR`/`2`. It retains USD currency anchors so immutable
CampusMarket orders and price history remain valid, appends reviewed EUR price
