...
export const createProductSchema = z.object({
...
}).refine(product => Boolean(product.name && product.hasInventory && product.image), {
  message: 'Name, quantity and image are required',
  params: { appCode: 'MISSING_PRODUCT_FIELDS', statusCode: 400 },
}).refine(product => !product.hasInventory || (Number.isInteger(product.inventory) && product.inventory >= 0), {
  message: 'Quantity must be a non-negative integer',
  params: { appCode: 'INVALID_PRODUCT_INVENTORY', statusCode: 400 },
}).refine(product => isProductStatus(product.status), {
  message: 'Product status must be draft, active, paused, sold_out, or archived',
  ...
  override: {
    ...
    properties: {
      ...
      quant: { type: 'integer', minimum: 0, description: 'Legacy alias for inventory' },
      image: { type: 'string' },
...
export const productFiltersSchema = z.object({
  q: z.unknown().optional().meta({ override: { type: 'string' } }),
  search: z.unknown().optional().meta({
    description: 'Alias for q',
    override: { type: 'string' },
  }),
  category: z.unknown().optional().meta({ override: { type: 'string' } }),
  subcategory: z.unknown().optional().meta({ override: { type: 'string' } }),
  seller: z.unknown().optional().meta({ override: { type: 'string' } }),
  status: z.unknown().optional().meta({
    override: { type: 'string', enum: productStatuses },
  }),
  availability: z.unknown().optional().meta({
    override: { type: 'string', enum: ['in_stock', 'sold_out'] },
  }),
  sort: z.unknown().optional().meta({
    override: {
      type: 'string',
  ...
  message: 'Product status filter must be draft, active, paused, sold_out, or archived',
  params: { appCode: 'INVALID_PRODUCT_STATUS_FILTER', statusCode: 400 },
}).refine(filters => !filters.availability || ['in_stock', 'sold_out'].includes(filters.availability), {
  message: 'Product availability filter must be in_stock or sold_out',
  params: { appCode: 'INVALID_PRODUCT_AVAILABILITY_FILTER', statusCode: 400 },
}).refine(filters => ['created_asc', 'created_desc', 'name_asc', 'inventory_desc'].includes(filters.sort), {
  message: 'Product sort must be created_asc, created_desc, name_asc, or inventory_desc',
...
export const productIdSchema = z.unknown().transform(value => requestString(value).trim())
...
export const sellerIdSchema = z.string().refine(id => mongoose.Types.ObjectId.isValid(id), {
