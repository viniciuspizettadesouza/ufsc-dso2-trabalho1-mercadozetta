...
function stripSensitiveFields<T extends { password?: string; tokenVersion?: number }>(userObject: T) {
...
export async function authenticate(body: LoginRequestBody | LoginCredentials, tenantId = defaultTenantId) {
  ...
  const user = await User.findOne({ tenantId, email }).select('+password +tokenVersion email username telephone tenantId');
...
export async function logout(userId: string, tenantId: string) {
  ...
  if (result.matchedCount !== 1)
    throw new AppError(401, 'INVALID_AUTH_TOKEN', 'Invalid authorization token');
}
