...
function loadAuthMiddleware(secret = 'test-secret', exists = vi.fn().mockResolvedValue({ _id: 'user-1' })) {
...
describe('auth middleware', () => {
    it('covers the complete token decision tree in one middleware instance', async () => {
        ...
        const next = vi.fn();
        const invoke = (authorization?: string, tenant?: { id: string }) => authMiddleware({
            headers: authorization ? { authorization } : {},
            tenant,
        }, {}, next);
        const sign = (payload: object) => `Bearer ${jwt.sign(payload, 'test-secret')}`;
        ...
        await invoke(sign({ id: 'user-1', tenantId: 'campus-market', tokenVersion: 1 }), { id: 'mercadozetta' });

        await invoke(sign({ id: 'user-1', tenantId: 'mercadozetta', tokenVersion: 0 }), { id: 'mercadozetta' });
        ...
        await invoke(sign({ id: 'user-1', tenantId: 'mercadozetta', tokenVersion: 2 }));
        expect(exists).toHaveBeenLastCalledWith({
            _id: 'user-1',
            tenantId: 'mercadozetta',
            tokenVersion: 2,
        ...
        expect(next).toHaveBeenCalledWith(expect.objectContaining({ code: 'INVALID_AUTH_TOKEN' }));
    });
    ...
    it('sets req.userId for valid active sessions', async () => {
        const authMiddleware = loadAuthMiddleware();
        const token = jwt.sign({ id: 'user-1', tenantId: 'mercadozetta', tokenVersion: 0 }, 'test-secret');
        const req: any = {
    ...
    it('rejects valid tokens from another tenant', async () => {
        const authMiddleware = loadAuthMiddleware();
        const token = jwt.sign({ id: 'user-1', tenantId: 'campus-market', tokenVersion: 0 }, 'test-secret');
        const req = {
            headers: { authorization: `Bearer ${token}` },
            tenant: { id: 'mercadozetta' },
        };
    ...
    it('rejects signed tokens without a valid user id', async () => {
        ...
        for (const payload of [{ tenantId: 'mercadozetta' }, { id: '' }, { id: 123 }]) {
    ...
    it('rejects tokens without tenant or session version claims', async () => {
        ...
        for (const payload of [{ id: 'user-1', tokenVersion: 0 }, { id: 'user-1', tenantId: 'mercadozetta' }]) {
            const token = jwt.sign(payload, 'test-secret');
            await authMiddleware({
                headers: { authorization: `Bearer ${token}` },
                tenant: { id: 'mercadozetta' },
