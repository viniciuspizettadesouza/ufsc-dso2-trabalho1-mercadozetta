...
describe('security config', () => {
  ...
  it('uses the development fallback secret only in local environments', () => {
    expect(loadSecurityWithEnv({ NODE_ENV: 'test' }).getJwtSecret()).toBe(
      'mercadozetta-dev-secret',
    );
    expect(
      loadSecurityWithEnv({ NODE_ENV: 'development' }).getJwtSecret(),
    ).toBe('mercadozetta-dev-secret');
  });
