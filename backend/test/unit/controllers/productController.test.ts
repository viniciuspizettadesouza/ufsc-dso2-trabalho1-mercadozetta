...
function loadController(service: any = {}) {
  if (service.listProducts)
    vi.spyOn(ProductService, 'listProducts').mockImplementation(
      service.listProducts,
    );
  if (service.getProductById)
    vi.spyOn(ProductService, 'getProductById').mockImplementation(
      service.getProductById,
    );
  if (service.createProduct)
    vi.spyOn(ProductService, 'createProduct').mockImplementation(
      service.createProduct,
    );
  if (service.listProductsBySeller)
    vi.spyOn(ProductService, 'listProductsBySeller').mockImplementation(
      service.listProductsBySeller,
    );
  return controller;
}
...
describe('productController', () => {
  ...
  it('lists products by seller with validated params and query', async () => {
    ...
    expect(listProductsBySeller).toHaveBeenCalledWith(
      'user-1',
      'mercadozetta',
      req.validated.query,
    );
