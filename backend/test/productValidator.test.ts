...
import { validateCreateProductPayload, validateProductFilters, validateProductId, validateSellerId } from '../src/validators/productValidator';
...
describe('product validator', () => {
  it('validates create-product payloads and filters with the same rules as the controllers', () => {
    ...
    expect(() => validateCreateProductPayload({ name: 'x', inventory: -1, image: 'img.png' })).toThrow(AppError);
    expect(() => validateProductFilters({ status: 'invalid' })).toThrow(AppError);
